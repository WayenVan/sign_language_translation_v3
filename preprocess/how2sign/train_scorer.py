...
Usage::
    ...
    # default: fit on preprocessed/scorer_features, save to outputs/hand_patch_scorer_how2sign
    python preprocess/how2sign/train_scorer.py

    # a different feature cache, and a wider ignore ring
    python preprocess/how2sign/train_scorer.py \\
        --features dataset/how2sign-front-clips/preprocessed/scorer_features_L8 --ring-radius 2 \\
        --out outputs/hand_patch_scorer_how2sign_L8

The frame dataset and features come from
``preprocess/how2sign/build_scorer_dataset.py`` and
``preprocess/how2sign/extract_scorer_features.py``.  Landmarks and features
are both taken on the same stored 224 crop (centre by default for How2Sign), so
a joint's normalized coordinate times the grid side is its patch, exactly as for
PHOENIX14T.  The fitted feature statistics are How2Sign's own, so a model
trained on How2Sign should point its ``scorer_path`` here rather than at a
PHOENIX14T or CSL-Daily scorer.

Held-out frames come from source videos that contributed nothing to the fit --
the frame dataset splits on ``video_id``, not on the sentence clip -- so recall
below is not reported on near-copies of the fitting frames.
...
def load_split(args, split: str) -> tuple[np.memmap, np.ndarray, np.ndarray]:
    ...
    if digest != info["id_sha1"]:
        raise RuntimeError(
            ...
            "wrong label. Re-run preprocess/how2sign/extract_scorer_features.py."
        )
...
def main():
    ...
    parser.add_argument(
        "--features",
        type=Path,
        default=Path("dataset/how2sign-front-clips/preprocessed/scorer_features"),
    )
    parser.add_argument(
        "--out", type=Path, default=Path("outputs/hand_patch_scorer_how2sign")
    )
