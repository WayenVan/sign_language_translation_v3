...
Usage::
    ...
    # default: C-RADIOv4-SO400M last layer over preprocessed/scorer_dataset
    python preprocess/how2sign/extract_scorer_features.py
    ...
    python preprocess/how2sign/extract_scorer_features.py \
        ...
        --out dataset/how2sign-front-clips/preprocessed/scorer_features_L8

    # smoke test: 128 frames per split, marked truncated in meta.json
    python preprocess/how2sign/extract_scorer_features.py --limit-frames 128 --out /tmp/check

The frame dataset comes from ``preprocess/how2sign/build_scorer_dataset.py``;
its frames are already the 224x224 eval crop, so they go to the backbone as is.

The scorer is one linear map over a single patch feature, so fitting it is a
matter of seconds -- but every fit needs the same ~4 million patch vectors, and
...
is otherwise a second of numpy.  The gigabytes that sampling would have saved
are not worth that.
...
Output::
    ...
    dataset/how2sign-front-clips/preprocessed/scorer_features/
...
def main():
    ...
    if not dataset_info_path.exists():
        raise FileNotFoundError(
            ...
            "preprocess/how2sign/build_scorer_dataset.py"
        )
