...
Usage::
    ...
    # default: fit on preprocessed/scorer_features, save to outputs/hand_patch_scorer_csl
    python preprocess/csl_daily/train_scorer.py

    # a different feature cache, and a wider ignore ring
    python preprocess/csl_daily/train_scorer.py \\
        --features dataset/CSL-Daily-HG/preprocessed/scorer_features_L8 --ring-radius 2 \\
        --out outputs/hand_patch_scorer_csl_L8

The frame dataset and features come from
``preprocess/csl_daily/build_scorer_dataset.py`` and
``preprocess/csl_daily/extract_scorer_features.py``.  Landmarks and features
are both taken on the stored bottom-aligned 224 crop, so a joint's normalized
coordinate times the grid side is its patch, exactly as for PHOENIX14T.  The
fitted feature statistics are CSL-Daily's own, so a model trained on CSL-Daily
should point its ``scorer_path`` here rather than at a PHOENIX14T scorer.
...
def load_split(args, split: str) -> tuple[np.memmap, np.ndarray, np.ndarray]:
    ...
    if digest != info["id_sha1"]:
        raise RuntimeError(
            ...
            "wrong label. Re-run preprocess/csl_daily/extract_scorer_features.py."
        )
...
def main():
    ...
    parser.add_argument(
        "--features",
        type=Path,
        default=Path("dataset/CSL-Daily-HG/preprocessed/scorer_features"),
    )
    parser.add_argument(
        "--out", type=Path, default=Path("outputs/hand_patch_scorer_csl")
    )
