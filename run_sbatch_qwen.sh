...
accelerate launch --num_processes=3 --mixed_precision=bf16 --debug -m csi_slt.commands.train \
  model=qwen3-14b-dino-base-tsamplerv2 \
  engine.training_args.output_dir=outputs/qwen3-14b-dino-base-tsamplerv2-shuffle-2 \
  model.config.visual_adapter_kwargs.num_layers_connector=2 \
