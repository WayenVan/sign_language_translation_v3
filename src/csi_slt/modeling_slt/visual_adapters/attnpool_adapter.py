...
class SpatiotemporalSeparableConv(nn.Module):
    ...
    from the first step. Either block can be disabled; at least one must stay
    on, and when both would be off the adapter drops this module entirely
    rather than carrying an identity with no parameters.
    """
    ...
    def __init__(
        ...
        if not isinstance(use_spatial_conv, bool):
            raise TypeError("use_spatial_conv must be a boolean")
        if not isinstance(use_temporal_conv, bool):
            raise TypeError("use_temporal_conv must be a boolean")
        if not (use_spatial_conv or use_temporal_conv):
            raise ValueError(
                "at least one of use_spatial_conv and use_temporal_conv must be "
                "True; build the adapter without patch_context instead"
            )
        ...
        self.spatial_norm = nn.LayerNorm(hidden_dim) if use_spatial_conv else None
        self.spatial_conv = (
            nn.Conv2d(
                hidden_dim,
                hidden_dim,
    ...
    def forward(self, patch_features: Tensor, visual_length: Tensor) -> Tensor:
        ...
        if self.spatial_conv is not None:
            features = features + self._spatial_block(
                self.spatial_norm(features), grid_height, grid_width
            )
...
class AttnPoolAdapter(nn.Module):
    ...
    def __init__(
        ...
        # With neither convolution enabled the module would be a parameter-free
        # identity, so it is dropped instead: the ablation arm that turns both
        # off should carry no patch-context parameters at all.
        self.patch_context = (
            SpatiotemporalSeparableConv(
                hidden_dim=input_dim,
                spatial_kernel_size=spatial_conv_kernel_size,
                temporal_kernel_size=temporal_conv_kernel_size,
                use_spatial_conv=use_spatial_conv,
                use_temporal_conv=use_temporal_conv,
                grid_size=patch_grid_size,
                debug_validation=debug_validation,
            )
            if (use_spatial_conv or use_temporal_conv)
            else None
        )
    ...
    def forward(
        ...
        contextualized_patches = (
            ...
            else self.patch_context(patch_features, visual_length)
        )
