...
itself the identity -- while still training from the first step, because a
linear layer's weight gradient depends on its input, not on the weight's own
value, so a zero weight is not a zero gradient.

That "from the first step" is a statement about the zero-initialized layers
themselves, not about every parameter in the block. At step 0 the backward
signal reaching anything *upstream* of one of those layers is multiplied by
its zero weight, so ``attn_norm``, the attention's ``in_proj``, ``mlp_norm``
and the MLP's hidden layer all see exactly zero gradient. One optimizer step
moves the output projections off zero and the rest of the block starts
training normally. This is the ordinary behavior of a zero-initialized
residual branch (ReZero, LayerScale, ControlNet) and is the price of an exact
identity at init; what matters is that the block is not a dead end, which the
nonzero gradient on the output projections is what guarantees.
...
class _TransformerContextBlock(nn.Module):
    def __init__(self, hidden_dim: int, num_heads: int, mlp_ratio: float, dropout: float):
        ...
        # these two layers still receive gradient from the first step -- see
        # the module docstring for what that does and does not cover, and
        # SpatiotemporalSeparableConv, which established the idiom here.
