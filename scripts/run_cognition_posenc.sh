...
#SBATCH --job-name=slt_qwen3_4b_nextframe_handroi_cls_20m_projdrop05_posenc_learned
...
if [[ "$DEBUG" == true ]]; then
  ...
  export WANDB_TAGS="next-frame,hand-roi,cls,20m,fixed-prompt,proj-dropout,posenc-learned"
  REPORT_TO=wandb
  OUTPUT_DIR="outputs/v5.0-qwen3-4b-cradio-l-nextframe-handroi-cls-20m-gate1-hardmatch-wr3-projdrop0.5-posenc-learned-0906.224x224"
fi
...
CMD_ARGS=(
  ...
  # Positional-encoding ablation on top of the best adapter.
  #
  ...
  #
  # The only change here is model.config.visual_position_embedding_type, edited
  # by hand on the line below. Every other flag is copied verbatim from
  # run_cognition_new.sh so this differs from the reference by one variable.
  #
  #   none    -- reference behaviour: nothing added, temporal order left
  #              entirely to the LLM's RoPE over the visual-token sequence.
  #              base.yaml's default; every v5.0 run so far used it.
  #   learned -- trainable MAX_TOKEN_LENGTH x 2560 table added before the tokens
  #              enter the LLM (RoPE still applies on top). Needs
  #              visual_position_embedding unfrozen or the table stays at init.
  #   sincos  -- fixed L2-normalized sinusoidal table, no parameters; its tail
  #              cannot get stuck at init on rarely-reached positions.
  #
  # To switch variant, edit three things by hand: this flag, the two
  # posenc-<...> strings up top (job-name / WANDB_TAGS / OUTPUT_DIR), and -- for
  # none / sincos -- optionally drop the parameter_mode=full line below (it is a
  # harmless no-op there, the module is None).
  #
  ...
  # --- the ablation variable: none | learned | sincos ---
  model.config.visual_position_embedding_type=learned
  # base.yaml freezes visual_position_embedding ("no module is constructed
  # while ...type=none"). The learned table only exists and only trains when
  # this is full.
