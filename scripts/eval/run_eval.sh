...
#
# Usage:
#   bash scripts/eval/run_eval.sh <CKPT_DIR> multi                 # de+en+zh, canonical prompts
#   bash scripts/eval/run_eval.sh <CKPT_DIR> de                    # one target language
#   bash scripts/eval/run_eval.sh <CKPT_DIR> multi share           # in-repo dataset, no staging
#   bash scripts/eval/run_eval.sh <CKPT_DIR> multi dry-run         # print the command only
#
...
#
# Help: scripts/eval/run_eval.sh --help
#
# One checkpoint evaluation = one prompt condition = one output directory.
# This is the parameterized worker behind scripts/eval/run_eval_*_prompt.sh;
# those pass EVAL_PROMPT_BANK / EVAL_PROMPT_IDS / EVAL_OUTPUT_DIR per prompt
# variant. Evaluation runs locally (no Slurm anywhere in scripts/eval).
...
# Host allowlist, same contract as scripts/run_llm_lora_inject.sh: an
...
# scripts/run_cognition_eval.sh carries a hand-copied
