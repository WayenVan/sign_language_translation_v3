...
#
# Usage:
#   bash scripts/eval/sweep4b/run_multilang_unseen_sweep.sh            # 已跑完 diverse 的每个 ckpt
#   bash scripts/eval/sweep4b/run_multilang_unseen_sweep.sh share      # 用仓库内数据集
#   bash scripts/eval/sweep4b/run_multilang_unseen_sweep.sh dry-run    # 只打印命令
#
# Environment:
...
#   EVAL_NUM_PROCESSES / EVAL_LANGUAGE_DISTRIBUTION 等透传给 suite 与 scripts/eval/run_eval.sh
#
# Help: scripts/eval/sweep4b/run_multilang_unseen_sweep.sh --help
#
...
#
#   outputs/eval/4b-multilang-diverse-eval/<run>/<checkpoint-step>/unseen/
#
...
if [[ ! -f "$PROJECT_DIR/scripts/eval/$SUITE_SCRIPT" ]]; then
  echo "suite launcher not found: $PROJECT_DIR/scripts/eval/$SUITE_SCRIPT" >&2
  exit 3
fi
...
echo "Sweep: 4B multilingual, held-out instruction suite ($SUITE)"
...
for checkpoint_dir in "${CHECKPOINTS[@]}"; do
  ...
  EVAL_OUTPUT_ROOT="$SWEEP_OUTPUT_ROOT" \
    bash "$PROJECT_DIR/scripts/eval/$SUITE_SCRIPT" "$checkpoint_dir" \
