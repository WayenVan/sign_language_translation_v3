...
#
# Usage:
#   bash scripts/stage2_14b/run_multilingual_lora_injection.sh             # sequential
#   bash scripts/stage2_14b/run_multilingual_lora_injection.sh --sbatch    # submit 2 Slurm jobs
#   bash scripts/stage2_14b/run_multilingual_lora_injection.sh --dry-run   # preview only
#
...
echo "Training: epochs=$EPOCHS; llm_lr=${LLM_LORA_LR:-1e-4}; optimizer=stable_adamw; GPUs=2"
...
for index in "${!PROMPT_MODES[@]}"; do
  ...
  job_name="slt_14b_llora_multi_${prompt_mode}_r${RANK}a${ALPHA}_stable"
