...
#
# Usage:
#   bash scripts/ph14t/eval/sweep4b/run_multilang_diverse_sweep.sh            # 每个 run 里最好的 ckpt
#   bash scripts/ph14t/eval/sweep4b/run_multilang_diverse_sweep.sh share      # 用仓库内数据集
#   bash scripts/ph14t/eval/sweep4b/run_multilang_diverse_sweep.sh dry-run    # 只打印命令
#
# Environment:
#   CHECKPOINT_STEPS="34593 26610"   手动指定 step，跳过自动选择
#   ALL_CHECKPOINTS=1                每个 run 目录下的全部 checkpoint 都跑
#   SWEEP_OUTPUT_ROOT                默认 outputs/eval/4b-multilang-diverse-eval
#   PROMPT_VARIANTS / FORCE / EVAL_NUM_PROCESSES 等透传给 scripts/ph14t/eval/run_eval.sh
#
# Help: scripts/ph14t/eval/sweep4b/run_multilang_diverse_sweep.sh --help
#
# 对两个 4B 多语言 stage-2 LoRA run（fixed-prompt 训练的和 diverse-prompt 训练的）
...
#
#   outputs/eval/4b-multilang-diverse-eval/<run>/<checkpoint-step>/diverse/
...
SUITE_SCRIPT="$PROJECT_DIR/scripts/ph14t/eval/run_eval_diverse_prompt.sh"
...
RUN_DIRS=(
  "$CKPT_ROOT/llmlora-v5.0-qwen3-4b-cradio-l-best-adapter-multilang-fixed_prompt-ep40-0909.224x224-checkpoint-184000-multilang-qkvo-r512a1024-fixed"
  "$CKPT_ROOT/llmlora-v5.0-qwen3-4b-cradio-l-best-adapter-multilang-diverse_train-ep40-0909.224x224-checkpoint-132000-multilang-qkvo-r512a1024-diverse"
)
...
echo "Sweep: 4B multilingual, diverse prompt suite (8 variants each)"
