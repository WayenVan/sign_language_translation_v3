...
#
# Usage:
#   bash scripts/ph14t/eval/sweep14b/run_multilang_counterfactual_sweep.sh            # 已跑完 diverse 的每个 ckpt
#   bash scripts/ph14t/eval/sweep14b/run_multilang_counterfactual_sweep.sh share      # 用仓库内数据集
#   bash scripts/ph14t/eval/sweep14b/run_multilang_counterfactual_sweep.sh dry-run    # 只打印命令
#
# Environment:
...
#   FORCE / EVAL_NUM_PROCESSES / EVAL_LANGUAGE_DISTRIBUTION 等透传给 suite 与 scripts/ph14t/eval/run_eval.sh
#
# Help: scripts/ph14t/eval/sweep14b/run_multilang_counterfactual_sweep.sh --help
#
...
#
#   outputs/eval/14b-multilang-diverse-eval/<run>/<checkpoint-step>/
...
for suite in "${SUITES[@]}"; do
  if [[ ! -f "$PROJECT_DIR/scripts/ph14t/eval/run_eval_${suite}_prompt.sh" ]]; then
    echo "suite launcher not found: $PROJECT_DIR/scripts/ph14t/eval/run_eval_${suite}_prompt.sh" >&2
    exit 3
  fi
done
...
echo "Sweep: 14B multilingual, counterfactual instruction switching (${SUITES[*]})"
...
for checkpoint_dir in "${CHECKPOINTS[@]}"; do
  ...
  for suite in "${SUITES[@]}"; do
    ...
    EVAL_OUTPUT_ROOT="$SWEEP_OUTPUT_ROOT" \
      bash "$PROJECT_DIR/scripts/ph14t/eval/run_eval_${suite}_prompt.sh" "$checkpoint_dir" \
