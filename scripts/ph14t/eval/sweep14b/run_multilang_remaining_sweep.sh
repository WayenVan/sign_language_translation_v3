...
#
# Usage:
#   bash scripts/ph14t/eval/sweep14b/run_multilang_remaining_sweep.sh            # 已跑完 diverse 的每个 ckpt
#   bash scripts/ph14t/eval/sweep14b/run_multilang_remaining_sweep.sh share      # 用仓库内数据集
#   bash scripts/ph14t/eval/sweep14b/run_multilang_remaining_sweep.sh dry-run    # 只打印命令
#
# Environment:
...
#
# Help: scripts/ph14t/eval/sweep14b/run_multilang_remaining_sweep.sh --help
#
...
#
#   outputs/eval/14b-multilang-diverse-eval/<run>/<checkpoint-step>/{unseen,wrong_task,unrelated}/
#
...
for suite in "${SUITES[@]}"; do
  ...
  if [[ ! -f "$PROJECT_DIR/scripts/ph14t/eval/$script" ]]; then
    echo "suite launcher not found: $PROJECT_DIR/scripts/ph14t/eval/$script" >&2
    exit 3
  fi
...
echo "Sweep: 14B multilingual, remaining suites: ${SUITES[*]}"
...
for checkpoint_dir in "${CHECKPOINTS[@]}"; do
  ...
  for suite in "${SUITES[@]}"; do
    ...
    EVAL_OUTPUT_ROOT="$SWEEP_OUTPUT_ROOT" \
      bash "$PROJECT_DIR/scripts/ph14t/eval/${SUITE_SCRIPTS[$suite]}" "$checkpoint_dir" \
