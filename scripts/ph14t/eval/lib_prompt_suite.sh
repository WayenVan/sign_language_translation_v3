...
# be run directly: scripts/ph14t/eval/run_eval_*_prompt.sh source it and call
#
...
if [[ ! -d "$PROJECT_DIR" ]]; then
  # tubbs checkout; scripts/ph14t/eval/run_eval.sh resolves the same two hosts.
...
run_prompt_suite() {
  ...
  local worker="$PROJECT_DIR/scripts/ph14t/eval/run_eval.sh"
  if [[ ! -f "$worker" ]]; then
  ...
  for arg in "$@"; do
    ...
    de | en | zh)
      if [[ "$allow_languages" != true ]]; then
        ...
        # keep apart -- the same reason scripts/ph14t/run_llm_lora_inject.sh refuses
