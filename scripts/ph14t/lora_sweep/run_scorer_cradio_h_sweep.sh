...
#
# Usage:
#   sbatch scripts/ph14t/lora_sweep/run_scorer_cradio_h_sweep.sh              # -9 -8 -10 -7 -11
#   sbatch scripts/ph14t/lora_sweep/run_scorer_cradio_h_sweep.sh -9           # one layer
#   sbatch scripts/ph14t/lora_sweep/run_scorer_cradio_h_sweep.sh -9 force     # refit, reuse the feature cache
#
...
#   1. preprocess/ph14t/extract_scorer_features.py -- the only GPU step: one pass of
#      the backbone over dataset/ph14_train_scorer_dataset, every patch cached
#      as float16 (~6.5 GB per layer at 1280-d).
#   2. preprocess/ph14t/train_scorer.py -- the linear fit, with the SO400M sweep's
